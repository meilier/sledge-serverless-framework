...
run_experiments() {
	...
	for conn in ${concurrency[*]}; do
		...
		hey -n "$iterations" -c "$conn" -cpus 1 -o csv -m GET "http://$hostname:10000" > "$results_directory/con$conn.csv" 2> /dev/null || {
			printf "[ERR]\n"
...
experiment_client 127.0.0.1 /root/github/sledge-serverless-framework/runtime/experiments/concurrency/res-single
